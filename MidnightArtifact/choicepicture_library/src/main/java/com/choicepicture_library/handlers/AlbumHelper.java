package com.choicepicture_library.handlers;
...
import com.choicepicture_library.model.ImageBucket;
...
public class AlbumHelper {
	...
	public void init(Context context) {
		if (this.context == null) {
			...
			cr = context.getContentResolver();//访问ContentProvider之前的初始化，获取操作对象
		}
	}
...
//				Log.e(TAG, "----- " + image.imageId + ", " + image.imagePath + ", " + image.thumbnailPath);
			}
		}
...
//		Log.e(TAG, "use time: " + (endTime - startTime) + " ms");
	...
	public List<ImageItem> getImagesList(boolean refresh) {
		if (refresh || (!refresh && !hasBuildImagesBucketList)) {
			buildImagesBucketList();
		}
		...
		while (itr.hasNext()) {
			Entry<String, ImageBucket> entry = (Entry<String, ImageBucket>) itr.next();
			tmpList.addAll(entry.getValue().imageList);
		}
		return tmpList;
	}
